...
func FindWorkersInBBoxProto(rd *redis.Client, minLat, minLng, maxLat, maxLng float64) ([]*models.CommandProto, error) {
	...
	if err != nil {
		...
		if errors.Is(err, redis.Nil) {
			return []*models.CommandProto{}, nil
		}
	...
	if len(locations) == 0 {
		return []*models.CommandProto{}, nil // No workers found in the bbox
	}
	...
	return workerProto, nil
}

func GetAllWorkersPaginatedProto(rd *redis.Client, page, pageSize int) ([]*models.CommandProto, int, error) {
	ctx := context.Background()
	...
	if totalWorkers == 0 {
		return []*models.CommandProto{}, 0, nil
	}
	...
	if start >= totalWorkers {
		return []*models.CommandProto{}, totalWorkers, nil
	}
